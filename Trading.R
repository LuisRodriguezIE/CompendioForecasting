...
# https://www.youtube.com/watch?v=hKDalfhDawA&index=68&list=PLNeOBuHe1dOKvuS1H7cWgiN5Ev1lJ3soy&t=2742s
...
dataA<- as.data.table(na.omit(getSymbols("EURUSD=X",src="yahoo",auto.assign = F),"extend"))
data1<-as.data.frame(dataA)
rm(dataA)
colnames(data1)<-c("Date","Open","High","Low","Close","Volume","Adjusted")
...
data<-read.csv("EURUSD60_5.csv",header = TRUE)
sapply(data, class)
